...
const MyGraphSecond = () => {
    ...
    function createVerticalLines(chart, container) {
        ...
        function createLineWithText(chartWidth, xPercent, color, isDashed, text) {
            ...
            textElement.setAttribute('y', '52%'); // Вы можете скорректировать это значение
    ...
    useEffect(() => {
        ...
        if (chartType === 'candlestick') {
            data = data.sort((a, b) => {
                const timeA = new Date(a.time.year, a.time.month - 1, a.time.day).getTime();
                const timeB = new Date(b.time.year, b.time.month - 1, b.time.day).getTime();
                return timeA - timeB;
            });
        ...
        const numberOfVisibleCandles = 600 / candleWidth; // candleWidth - это ширина одной свечи в пикселях
        const positionToScroll = middleIndex - (numberOfVisibleCandles / 2);
        chart.timeScale().scrollToPosition(-positionToScroll, false);


        const markers = getMarkers(data, series, 'candlestick');
        ...
        series.setMarkers(markers);
            ...
            const markers = getMarkers(data, series, 'line');
        ...
        //
        // addVerticalLine(chart, lockTime);
        // addVerticalLine(chart, expirationTime);

        const interval = setInterval(() => {
            // Генерация нового времени
            const newTime = new Date(data[data.length - 1].time.year, data[data.length - 1].time.month - 1, data[data.length - 1].time.day + 1);

            if (chartType === 'candlestick') {
                // Генерация данных для свечного графика
                const newPrice = data[data.length - 1].close + (Math.random() * 2 - 1);
                const newData = {
                    time: {
                    ...
                    open: newPrice,
                    high: newPrice + Math.random(),
                    low: newPrice - Math.random(),
                    close: newPrice + Math.random() - 0.5,
                };
                series.update(newData); // Обновление свечного графика
                data.push(newData); // Добавление в массив данных
            } else if (chartType === 'line') {
                ...
                series.update(newData); // Обновление линейного графика
                data.push({ ...newData, open: newPrice, high: newPrice, low: newPrice, close: newPrice }); // Добавление в массив данных с соответствующей структурой
            }
        }, 1000);
...
export default MyGraphSecond;

function isDataSorted(data) {
    for (let i = 1; i < data.length; i++) {
        const timeA = new Date(data[i - 1].time.year, data[i - 1].time.month - 1, data[i - 1].time.day).getTime();
        const timeB = new Date(data[i].time.year, data[i].time.month - 1, data[i].time.day).getTime();
...
function getMarkers(data, series, chartType) {
    ...
    for (let i = 1; i < datesForMarkers.length; i++) {
        if (chartType === 'candlestick') {
            ...
            if (datesForMarkers[i].close < datesForMarkers[indexOfMinPrice].close) {
                indexOfMinPrice = i;
            }
        }
    }
    ...
    for (let i = 0; i < datesForMarkers.length; i++) {
        const markerValue = chartType === 'candlestick' ? datesForMarkers[i].high : datesForMarkers[i].close;
