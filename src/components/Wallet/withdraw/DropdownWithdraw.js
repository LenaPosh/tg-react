...
export const DropdownWithdraw = ({ onTokenChangeWithdraw, updateDepositText2 }) => {
    ...
    const usdtOptionsWithdraw = [
        { network: 'TRC20', minAmount: '2.0' },
        { network: 'ERC20', minAmount: '5.0' }
    ];

    const usdtDefaultOption = usdtOptionsWithdraw[0];

    const [selectedOption, setSelectedOption] = useState(optionsWithdraw[0]);
    ...
    useEffect(() => {
        let displayMinAmount = isUSDTSelected
            ? usdtOptionsWithdraw.find(option => option.network === selectedUSDTNetwork)?.minAmount
            : selectedOption.minAmount;

        displayMinAmount = selectedOption.value === 'WALLETCONNECT' ? '0.01 ETH' : displayMinAmount;

        updateDepositText2(`Select the token to deposit (Min ${displayMinAmount} ${selectedOption.value === 'WALLETCONNECT' ? 'ETH' : isUSDTSelected ? 'USDT' : selectedOption.label})`);
    }, [selectedOption, selectedUSDTNetwork, isUSDTSelected, updateDepositText2]);
    ...
    const handleOptionClick = (option) => {
        ...
        setIsOpen(false);
        onTokenChangeWithdraw(option.value, option.label, option.minAmount);
        ...
        <div className={`dropdown-wallet ${isOpen ? 'isOpen' : ''}`}>
            ...
            {isOpen && (
                <ul className="dropdown-menu-wallet">
                    {optionsWithdraw.filter(option => option.value !== selectedOption.value).map((option, index) => (
